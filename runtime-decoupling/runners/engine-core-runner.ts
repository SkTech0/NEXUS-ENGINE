#!/usr/bin/env node
/**
 * Engine-core standalone runner.
 * Independent process: HTTP server, own config, health/readiness/liveness.
 */
...
async function handleRequest(
  ...
  config: ReturnType<typeof loadEngineConfig>
): Promise<void> {
  const url = req.url ?? '/';
  ...
  if (method === 'POST' && (url === '/api/execute' || url === '/execute')) {
    ...
    return send(res, 200, { success: true, payload: { action, parameters } });
  }
