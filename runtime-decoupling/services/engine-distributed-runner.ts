/**
 * Runner for engine-distributed-service. Starts process, loads config, registers, health.
 */
...
const DEFAULT_PORT = 5016;
const REPO_ROOT = process.env['NEXUS_REPO_ROOT'] || path.resolve(__dirname, '../..');
const SERVICE_DIR = path.join(REPO_ROOT, 'services', 'engine-distributed-service');
...
export function getConfig(): { host: string; port: number } {
  return {
    ...
    port: Number(process.env['ENGINE_DISTRIBUTED_SERVICE_PORT']) || DEFAULT_PORT,
  };
}
...
export function startProcess(): ChildProcess {
  ...
  child = spawn(process.env['PYTHON_PATH'] || 'python3', ['run.py'], {
    cwd: SERVICE_DIR,
    env: { ...process.env, ENGINE_DISTRIBUTED_SERVICE_PORT: String(config.port), PYTHONPATH: SERVICE_DIR },
    stdio: 'inherit',
  });
