/**
 * Runner for engine-data-service. Starts process, loads config, registers, health.
 */
...
const SERVICE_DIR = path.join(REPO_ROOT, 'services', 'engine-data-service');
...
export function getConfig(): { host: string; port: number } {
  return {
    ...
    port: Number(process.env['ENGINE_DATA_SERVICE_PORT']) || DEFAULT_PORT,
  };
}
...
export function startProcess(): ChildProcess {
  ...
  child = spawn(process.env['PYTHON_PATH'] || 'python3', ['run.py'], {
    cwd: SERVICE_DIR,
    env: { ...process.env, ENGINE_DATA_SERVICE_PORT: String(config.port), PYTHONPATH: SERVICE_DIR },
    stdio: 'inherit',
  });
