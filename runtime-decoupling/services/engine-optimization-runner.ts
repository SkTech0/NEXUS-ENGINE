/**
 * Runner for engine-optimization-service. Starts process, loads config, registers, health.
 */
...
const SERVICE_NAME = 'engine-optimization-service';
const DEFAULT_PORT = 5013;
const REPO_ROOT = process.env['NEXUS_REPO_ROOT'] || path.resolve(__dirname, '../..');
const SERVICE_DIR = path.join(REPO_ROOT, 'services', 'engine-optimization-service');
...
export function getConfig(): { host: string; port: number } {
  return {
    ...
    port: Number(process.env['ENGINE_OPTIMIZATION_SERVICE_PORT']) || DEFAULT_PORT,
  };
}
...
export function startProcess(): ChildProcess {
  ...
  child = spawn(process.env['PYTHON_PATH'] || 'python3', ['run.py'], {
    cwd: SERVICE_DIR,
    env: { ...process.env, ENGINE_OPTIMIZATION_SERVICE_PORT: String(config.port), PYTHONPATH: SERVICE_DIR },
    stdio: 'inherit',
  });
