...
Each product lives under `products/<product-name>/` as a **product definition**: README, vision, scope, flows, personas, use-cases, demo scenarios, integration map, roadmap. Implementation lives in the platform (engine-api, product-ui), not inside the product folder.

---

## What products will exist

The repo is set up for **additive** products. New products = new folder under `products/` + docs, then (when needed) domain slices in the API and demo routes in the UI. There is no fixed list of “allowed” products; the structure supports loan, hiring, fraud, playground, and future domains (e.g. compliance, underwriting, pricing) as long as they fit the platform’s engine model.
...
- **Product identity lives in `products/`.** Each product folder owns: what it is, who it’s for, what’s in scope, how it evolves. Implementation details stay in engine-api and product-ui.
...
| **Where** | engine-*, engine-api, product-ui, infra, saas-layer, etc. | products/* (docs + scope); API/UI (domain and demo code) |
...
1. **Define:** Create a folder under `products/` with README, scope, vision, flows, personas, use-cases. No code required to “add” a product as a concept.
...
- **Roadmaps** live in each product folder (e.g. `products/nexus-loan-engine/roadmap.md`). They express direction and themes, not commitments.
...
Demos are the **visible** side of products. Product docs in `products/` define the narrative and scope; demo routes and components in `product-ui/src/app/demo/` implement the experience.
...
Domain logic lives in the API (and optionally engines); product identity and scope live in `products/`.
...
- Roadmaps in `products/*/roadmap.md` are **directional**: themes, opportunities, and possible features. They are not release plans or commitments.
...
When in doubt: if it’s about **what we offer and why** → product docs. If it’s about **how it runs and how to extend it** → platform and [DEV_GUIDE.md](./DEV_GUIDE.md) / [ARCH_OVERVIEW.md](./ARCH_OVERVIEW.md).
