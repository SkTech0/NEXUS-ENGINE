...
import type { EngineOptimizationServiceConfig } from './config';
...
async function handleRequest(
  ...
  config: EngineOptimizationServiceConfig
): Promise<void> {
  const url = req.url ?? '/';
...
export async function createServer(config: EngineOptimizationServiceConfig): Promise<http.Server> {
...
export async function startServer(server: http.Server, config: EngineOptimizationServiceConfig): Promise<void> {
  await runStartup();
