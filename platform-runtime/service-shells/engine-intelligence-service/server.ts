...
import type { EngineIntelligenceServiceConfig } from './config';
...
import { evaluate } from './adapter';

const SERVICE_NAME = 'engine-intelligence-service';
...
async function handleRequest(
  ...
  config: EngineIntelligenceServiceConfig
): Promise<void> {
  const url = req.url ?? '/';
  ...
  if (method === 'POST' && (url === '/api/evaluate' || url === '/evaluate')) {
    ...
    const out = await evaluate(body);
    return send(res, 200, out);
  }
  send(res, 404, { error: 'Not found' });
}

export async function createServer(config: EngineIntelligenceServiceConfig): Promise<http.Server> {
...
export async function startServer(server: http.Server, config: EngineIntelligenceServiceConfig): Promise<void> {
  await runStartup();
