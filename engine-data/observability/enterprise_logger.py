"""
Engine Data — Enterprise logger (ERL-4).
...
class EnterpriseLogger:
    """Structured logger with trace_id, correlation_id, standard schema."""
    ...
    def info(self, operation: str, entry: dict[str, Any] | None = None) -> None:
        d = self._entry(operation, "ok", **(entry or {}))
        self._log.info("%s", d)

    def warn(self, operation: str, entry: dict[str, Any] | None = None) -> None:
        d = self._entry(operation, "degraded", **(entry or {}))
        self._log.warning("%s", d)

    def error(self, operation: str, entry: dict[str, Any] | None = None) -> None:
        d = self._entry(operation, "error", **(entry or {}))
        self._log.error("%s", d)
