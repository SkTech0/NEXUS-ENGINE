...
# Build from repo root: docker build -f deployment-shells/docker/engine-api.Dockerfile .
FROM mcr.microsoft.com/dotnet/sdk:10.0 AS build
WORKDIR /src
COPY engine-api/src/EngineApi/EngineApi.csproj EngineApi/
RUN dotnet restore EngineApi/EngineApi.csproj
COPY engine-api/src/EngineApi EngineApi/
