...
cd engine-api/src/EngineApi && dotnet run
...
docker compose -f deployment-shells/compose/engine-api.yml up -d
docker compose -f deployment-shells/compose/product-ui.yml up -d

# Or run all (you’d add a combined compose file or script)
# See deployment-shells/compose/*.yml for each service
```
...
docker build -f deployment-shells/docker/engine-api.Dockerfile -t your-registry/nexus-engine-api:latest .
docker build -f deployment-shells/docker/product-ui.Dockerfile -t your-registry/nexus-product-ui:latest .
docker push your-registry/nexus-engine-api:latest
docker push your-registry/nexus-product-ui:latest

# 2. Optional ConfigMap
kubectl apply -f deployment-shells/k8s/nexus-env-configmap.yaml

# 3. Deploy (update image in YAML to your registry)
kubectl apply -f deployment-shells/k8s/engine-api-deployment.yaml
kubectl apply -f deployment-shells/k8s/product-ui-deployment.yaml
```
...
| Run API | `engine-api/` → `dotnet run` or service-shell |
| Deploy (Compose) | `deployment-shells/compose/*.yml` |
| Deploy (K8s) | `deployment-shells/k8s/*.yaml` |
