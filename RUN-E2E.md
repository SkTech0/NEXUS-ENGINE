...
```bash
npm install
cd engine-api && dotnet restore && cd ..
```
...
- UI proxies `/api` to http://localhost:5000 (see `product-ui/proxy.conf.json`).
...
# .NET API
cd engine-api && dotnet test && cd ..

# Python engines (optional; needs pytest)
pip install -r requirements-test.txt
# Then per engine, e.g.:
cd engine-ai && python3 -m pytest tests/ -v --tb=short && cd ..
```
...
```bash
node dist/runtime-decoupling/runners/engine-api-runner.js
```
...
- **Host A (e.g. cloud VM):** Install .NET 10, clone repo, run `dotnet run --project engine-api/src/EngineApi/EngineApi.csproj` (and expose port 5000).
...
| Install .NET deps | `cd engine-api && dotnet restore && cd ..` |
| Install Python test deps | `pip install -r requirements-test.txt` |
| Start API | `cd engine-api && dotnet run --project src/EngineApi/EngineApi.csproj` |
| Start UI | `npx nx serve product-ui` |
| Build runtime-decoupling | `npx nx build runtime-decoupling` |
| Run one decoupled engine | `node dist/runtime-decoupling/runners/engine-core-runner.js` (or serve-engine-ai, etc.) |
...
1. Install: Node 20+, .NET 10, then `npm install` and `cd engine-api && dotnet restore && cd ..`.
2. Terminal 1: `cd engine-api && dotnet run --project src/EngineApi/EngineApi.csproj`.
